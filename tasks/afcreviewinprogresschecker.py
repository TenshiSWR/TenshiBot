...
from tools.summaries import TASK2_NOTIFICATION, TASK2_SUMMARY
import toolforge
...
class AfcReviews:
    def __init__(self):
        site = pywikibot.Site()
        self.drafts = [draft for draft in pywikibot.Category(site, "Pending AfC submissions being reviewed now").articles()]
        ...
        self.notification_system.notify_all(TASK2_NOTIFICATION)
    ...
    def check_notified(self, user: str) -> bool:
        ...
        if time is None:  # It's not there if its None
            return False
        if datetime.utcnow().replace(tzinfo=None)-timedelta(hours=23) > time[0] > datetime.utcnow().replace(tzinfo=None)-timedelta(hours=25):  # A bit of leeway, but not much.
            return True
        else:
            return False
    ...
    def check_pending_afc_submissions(self):
        for draft in self.drafts:
            try:
                for template in parse(draft.text).filter_templates():
                ...
                if (template.name.matches("AfC submission") or template.name.matches("AFC submission")) and template.get(1).value == "r":
                    try:
                        ...
                        try:
                            ...
                            if (datetime.utcnow()-timedelta(hours=72)) > timestamp:
                                ...
                                draft.save(summary=TASK2_SUMMARY, minor=False, quiet=True)
                    else:
                        if (datetime.utcnow()-timedelta(hours=72)) > timestamp and self.check_notified(reviewer):
                            ...
                            draft.text = draft.text.replace(str(template), str(template).replace("r", "", 1))
                            draft.save(summary=TASK2_SUMMARY, minor=False, quiet=True)
                        elif (datetime.utcnow()-timedelta(hours=48)) > timestamp:
    ...
    def log_notifications(self):
        ...
        for reviewer in self.notification_system.notification_queue.keys():
            ...
            if time is None:
                ...
                self.cursor.execute("INSERT INTO long_reviews(user, time) VALUES(%(username)s, %(time)s);", {"username": reviewer, "time": datetime.utcnow()})
            else:
                print("Updating data for {}".format(reviewer))
                self.cursor.execute("UPDATE long_reviews SET time = %(time)s WHERE user = %(username)s;", {"time": datetime.utcnow(), "username": reviewer})
