...
while index < len(working_list):
    ...
    if not result:  # 1. Has to have "albums" in it
        ...
        continue
    result = re.search("==* *.*(Track listing|Track list|Tracklist|Disc 1|Disc 2|CD track listing|DVD track listing|Set listing|Tracks|Tracklisting|Disc listing|Disc one|Disc two)", working_text, re.I)
    if result:
        del text[working_list[index]]
        working_list.pop(index)
        continue
